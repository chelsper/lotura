...
function fallbackExcerpt(value) {
  if (typeof value !== "string") return value;
  const text = value.trim();
  if (text.length <= 1200) return text;
  const suffix = "… [Excerpt only; see the full answer in the evidence transcript.]";
  return text.slice(0, 1200 - suffix.length).trimEnd() + suffix;
}
...
export function readStoredDiscoveryAnalystResult(value, providerKey) {
  const validated = validateDiscoveryAnalystResult(value);
  if (validated) return validated;
  if (providerKey !== "deterministic-analyst-fallback"
    || !value || typeof value !== "object" || Array.isArray(value)) return null;

  // Older fallback snapshots copied full answers into these bounded display lists.
  // Adapt only that known format mismatch on read; never rewrite evidence/history
  // or relax validation of external AI output.
  return validateDiscoveryAnalystResult({
    ...value,
    clear: Array.isArray(value.clear) ? value.clear.map(fallbackExcerpt) : value.clear,
    needsValidation: Array.isArray(value.needsValidation)
      ? value.needsValidation.map(fallbackExcerpt)
...
export function createDiscoveryAnalystFallback(
  ...
  const excluded = new Set(
    Array.isArray(excludedPromptKeys) ? excludedPromptKeys : [],
  );
  const question = DISCOVERY_QUESTIONS.find(
    (candidate) => !covered.has(candidate.key) && !excluded.has(candidate.key),
  )
    ?? DISCOVERY_QUESTIONS.find((candidate) => !excluded.has(candidate.key))
  ...
  const needsValidation = observations
    ...
    .slice(-6)
    .map((observation) => fallbackExcerpt(observation.responseText || observation.promptText))
    .filter(Boolean);
  const known = observations
    .filter((observation) => observation.epistemicState === "known")
    .slice(-6)
    .map((observation) => fallbackExcerpt(observation.responseText))
    .filter(Boolean);
  return {
    ...
    nextQuestion: {
      ...
      rationale: reason === "provider_unavailable"
        ...
        : reason === "invalid_response"
          ? "Lotura could not validate the AI response, so this is a standard question. Your full answers remain saved."
        : reason === "timeout"
          ? "The AI response did not finish within the time limit, so this is a standard question. No automatic retry was made."
        : reason === "prohibited_or_oversized_context"
          ? "The interview context did not pass the pilot’s content or size checks, so this is a standard question. Your full answers remain saved."
        : reason === "different_question_requested"
          ? "You skipped the prior question, so Lotura moved to a different unresolved Discovery topic."
