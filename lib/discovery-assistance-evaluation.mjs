...
const UNCERTAINTY_QUESTION_PATTERN = /\b(?:do you know|if so|is it (?:clear|confirmed|documented|known)|may|might|needs? (?:confirmation|validation)|not sure|whether|who (?:can|could) (?:confirm|validate|verify)|what (?:can|could|would) (?:confirm|validate|verify))\b/i;
const NEUTRAL_VERIFICATION_QUESTION_PATTERN = /^(?:are|can|could|did|do|does|has|have|is|was|were)\b[^?]*\?$/i;
...
export function buildOpenAIDiscoveryEvaluationRequest(input) {
  ...
  return {
    ...
    text: {
      format: {
        name: "lotura_discovery_assistance_evaluation_v4",
...
function hasUncertainObservation(input) {
  return input.packet.sources.some((source) =>
    source.kind.endsWith("observation")
    && String(source.snapshot.epistemicState ?? "").trim() !== ""
    && source.snapshot.epistemicState !== "known");
}

function suggestionsPreserveUncertainty(input, suggestions) {
  if (input.assistanceKind === "question_suggestions") {
    return !hasUncertainObservation(input)
      || suggestions.every((suggestion) =>
        UNCERTAINTY_QUESTION_PATTERN.test(suggestion.suggestedText)
        || NEUTRAL_VERIFICATION_QUESTION_PATTERN.test(
...
function candidateChecks(input, suggestions) {
  ...
  return {
    ...
    preservesUncertainty: suggestionsPreserveUncertainty(input, suggestions),
    safeContent: (() => {
      try {
...
export function evaluateDiscoveryAssistanceSafety(input, suggestions) {
  return candidateChecks(input, suggestions);
}
...
export function evaluateDiscoveryAssistanceCandidate({ humanReview, input, outputText }) {
  ...
  const automatedChecks = evaluateDiscoveryAssistanceSafety(input, suggestions);
