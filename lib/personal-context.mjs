...
export function describePersonalContextDependency(processName, dependency) {
  const labels = {
  ...
  const source = dependency.direction === "incoming"
    ? dependency.processName
    : processName;
  const target = dependency.direction === "incoming"
    ? processName
    : dependency.processName;
  return `${source} ${labels[dependency.type] ?? dependency.type} ${target}`;
}
