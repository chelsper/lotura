...
- Ask at most three short, conversational questions, or return one clarity draft when requested.
...
export function buildOpenAIDiscoveryEvaluationRequest(input) {
  ...
  return {
    ...
    text: {
      format: {
        name: "lotura_discovery_assistance_evaluation",
...
export function parseOpenAIDiscoveryEvaluationOutput(input, outputText) {
  ...
  if (validated.assistanceKind === "clarity_draft" && parsed.suggestions.length !== 1) {
    throw new Error("A clarity evaluation must return exactly one draft.");
  }
...
function priorQuestions(input) {
  ...
  return new Set(questions.map(normalizeQuestion).filter(Boolean));
}
...
function candidateChecks(input, suggestions) {
  ...
  return {
    ...
    nonRepetitive: suggestions.every(
      (suggestion) => !existingQuestions.has(normalizeQuestion(suggestion.suggestedText)),
    ),
