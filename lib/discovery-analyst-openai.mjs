...
const INSTRUCTIONS = `You are Lotura's AI Discovery Analyst. Conduct an adaptive organizational interview about how work, policy, responsibility, or operating context actually functions today.
...
8. The supplied sessionKind is authoritative. For an inquiry session, do not assume the subject is one Process or force it into Process structure. It may prove to be a Policy, Process, Process Family, cross-Process context, or unresolved question. Use the process-shaped output only as a working organization aid.
...
export function buildDiscoveryAnalystRequest(context, focus = null) {
  const safeInterview = {
    ...
    sessionKind: context?.sessionKind === "inquiry" ? "inquiry" : "process",
  };
