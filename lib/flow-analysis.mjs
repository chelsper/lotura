    ...
    if (!role.primary) {
      results.push(
        finding({
          ...
          summary: `Direct impact: ${countPhrase(role.affectedProcessIds.length, "active process", "active processes")} currently ${role.affectedProcessIds.length === 1 ? "relies" : "rely"} on this role's intended responsibility.`,
          facts: [
  ...
  for (const role of roleImpacts) {
    ...
    findings.roles.push(
      finding({
        ...
        title: role.roleName,
        summary: `${role.roleName} owns ${countPhrase(role.ownedProcessIds.length, "process", "processes")} and is responsible for ${countPhrase(role.responsibleStepIds.length + role.inheritedStepIds.length, "step")} across ${countPhrase(role.affectedProcessIds.length, "process", "processes")}.`,
        facts: [
  ...
  for (const impact of analyzeSystemImpact(seed, graph)) {
    findings.systems.push(
      finding({
        ...
        summary: `${impact.systemName} is directly used by ${impact.directProcessIds.length} of ${countPhrase(activeProcessIds.size, "active process", "active processes")}.`,
        facts: [
...
function scenarioFindings(seed, roleImpacts, graph) {
  ...
  for (const role of roleImpacts) {
    roles.push({
      ...
      vacancy: finding({
        ...
        summary: `Direct impact: ${countPhrase(role.affectedProcessIds.length, "active process", "active processes")} would retain intended role ownership but lose a current primary assignee.`,
        facts: [
          fact("Processes", role.affectedProcessIds.length),
          fact("Responsible steps", role.responsibleStepIds.length + role.inheritedStepIds.length),
          fact("Exceptions owned", role.ownedExceptionIds.length),
  ...
  const systemScenarios = analyzeSystemImpact(seed, graph).map((impact) =>
    finding({
      ...
      summary: `Direct impact: ${countPhrase(impact.directProcessIds.length, "active process", "active processes")} ${impact.directProcessIds.length === 1 ? "uses" : "use"} this system. Potential indirect impact: ${countPhrase(impact.potentialIndirectIds.length, "downstream process", "downstream processes")} ${impact.potentialIndirectIds.length === 1 ? "is" : "are"} connected to those processes.`,
      facts: [
  ...
  const processScenarios = analyzeProcessChangeImpact(seed, graph).map((impact) => {
    ...
    return finding({
      ...
      summary: `Review recommended with ${countPhrase(impact.directUpstreamIds.length, "direct upstream process", "direct upstream processes")}, ${countPhrase(impact.directDownstreamIds.length, "direct downstream process", "direct downstream processes")}, ${countPhrase(impact.directSystemIds.length, "directly linked system")}, and ${countPhrase(impact.participatingRoleIds.length, "participating role")}.`,
      facts: [
